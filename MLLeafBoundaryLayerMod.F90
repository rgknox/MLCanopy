...
contains
  ...
  !-----------------------------------------------------------------------
  subroutine LeafBoundaryLayer (il, mlcanopy_inst)
    !
    ! !DESCRIPTION:
    ! Leaf boundary layer conductance
    !
    ...
    associate ( &
                                                   ! *** Input ***
    filter    => mlcanopy_inst%filter         , &  ! Which patches are active
    num_filter=> mlcanopy_inst%num_filter     , &  ! Number of active patches
