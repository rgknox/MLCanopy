module MLCanopyFluxesType
  ...
  use MLVarpar, only : nlevmlcan, nleaf,isun,isha
  !
  ! !PUBLIC TYPES:
  implicit none
  save
  ...
  type, public :: mlcanopy_type
    ...
    integer, pointer :: pft_wgt(:,:) ! Fractional weight of each pft
                                     ! Contained in this
  ...
  contains
    ...
    procedure, private :: NCanopy           ! Count number of canopies on this thread
...
contains
  ...
  function NCanopy(this) result(n_canopy)

    class(mlcanopy_type) :: this

    ! Simply count and return the number of canopies on this thread
    
    n_canopy = (this%endp-this%begp)+1
    
  end function NCanopy
  ...
  subroutine InitAllocate (this)
    ...
    associate(begp => this%begp ; endp = this%endp )

    ! Indexing and filtering
    allocate (this%active_filter(begp:endp)) this%active_filter(begp:endp) = spval
