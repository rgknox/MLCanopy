...
contains
  ...
  subroutine AllocatePhotoParams(begpft,endpft)
      
    integer,intent(in) :: begpft,endpft
    
    allocate(photo_params%c3psn(begpft:endpft))
    allocate(photo_params%g0_BB(begpft:endpft))
    allocate(photo_params%g1_BB(begpft:endpft))
    allocate(photo_params%g0_MED(begpft:endpft))
    allocate(photo_params%g1_MED(begpft:endpft))
    allocate(photo_params%psi50_gs(begpft:endpft))
    allocate(photo_params%shape_gs(begpft:endpft))
    allocate(photo_params%gsmin_SPA(begpft:endpft))
    allocate(photo_params%iota_SPA(begpft:endpft))

  end subroutine AllocatePhotoParams
...
contains
  ...
  subroutine LeafPhotosynthesis (num_filter, filter, il, pft, mlcanopy_inst)
    !
    ! !DESCRIPTION:
    ...
    integer, intent(in) :: pft(mlcanopy_inst%begp:) ! pft index
    ...
    do fp = 1, num_filter
       ...
       do ic = 1, ncan(p)
          ...
          if (dpai(p,ic) > 0._r8) then
             ...
             ! Select parameters for An-gs stomatal models
             ...
             case (0, 1)
                ...
                if (nint(c3psn(pft(p))) == 1) then
                   ci0 = 0.7_r8 * cair(p,ic)
                else if (nint(c3psn(pft(p))) == 0) then
             ...
             case (2)
                ...
                call StomataOptimization (p, ic, il, pft(p), mlcanopy_inst)
             ...
             select case (gs_type)
             case (0, 1)
                call CiFunc (p, ic, il, pft(p), mlcanopy_inst, 0._r8, ci(p,ic,il))
             case (2)
                call CiFuncGs (p, ic, il, pft(p), mlcanopy_inst, ci(p,ic,il))
             case default
                call endrun (msg=' ERROR: LeafPhotosynthesis: gs_type not valid')
             end select
       ...
       do ic = 1, ncan(p)
          ...
          if (dpai(p,ic) > 0._r8) then
             ...
             case (0)
                fpsi = 1._r8
             case (1)
                fpsi = 1._r8 / (1._r8 + (lwp(p,ic,il)/psi50_gs(pft(p)))**shape_gs(pft(p)))
             end select
             gs(p,ic,il) = max(gspot(p,ic,il)*fpsi, gsmin_SPA(pft(p)))
    ...
    do fp = 1, num_filter
       p = filter(fp)
       do ic = 1, ncan(p)
          call C13Fractionation (p, ic, il, pft(p), mlcanopy_inst)
       end do
    end do
  ...
  subroutine CiFuncGs (p, ic, il, ipft, mlcanopy_inst, ci_val)
    ...
    integer, intent(in) :: ic       ! Aboveground layer index
    integer, intent(in) :: il       ! Sunlit (1) or shaded (2) leaf index
    integer, intent(in)  ::  ipft   ! PFT index for the patch
    ...
    if (dpai(p,ic) > 0._r8) then
       ...
       case default
          call endrun (msg=' ERROR: CiFuncGs: colim_type not valid')
       end select
  ...
  subroutine StomataOptimization (p, ic, il, ipft, mlcanopy_inst)
    ...
    integer, intent(in) :: ic             ! Aboveground layer index
    integer, intent(in) :: il             ! Sunlit (1) or shaded (2) leaf index
    integer, intent(in) :: ipft           ! PFT index of the patch
    type(mlcanopy_type), intent(inout) :: mlcanopy_inst
    !
    ! !LOCAL VARIABLES:
  ...
  subroutine StomataEfficiency (p, ic, il, ipft, mlcanopy_inst, gs_val, check)
    ...
    hs = (gbv(p,ic,il)*eair(p,ic) + gs(p,ic,il)*leaf_esat(p,ic,il)) &
       / ((gbv(p,ic,il) + gs(p,ic,il)) * leaf_esat(p,ic,il))
    vpd = max(leaf_esat(p,ic,il) - hs * leaf_esat(p,ic,il), 0.1_r8)
  ...
  subroutine C13Fractionation (p, ic, il, ipft, mlcanopy_inst)
    ...
    integer, intent(in) :: ic       ! Aboveground layer index
    integer, intent(in) :: il       ! Sunlit (1) or shaded (2) leaf index
    integer, intent(in) :: ipft     ! PFT index for this patch
    type(mlcanopy_type), intent(inout) :: mlcanopy_inst
    !
    ! !LOCAL VARIABLES:
    !---------------------------------------------------------------------
